...
def prepare_chunks(mineru_parsed_dir: Path, chunks_output_dir: Path, paper_id: str = None):
    ...
    for paper_dir_level1 in tqdm(paper_dirs, desc="Preparing chunk files"):
        pid = paper_dir_level1.name
        if paper_id and pid != paper_id:
            continue
        paper_dir_level2 = paper_dir_level1 / pid / "auto"
        if not paper_dir_level2.exists():
            logger.warning(f"Could not find 'auto' directory in {paper_dir_level1 / pid}")
            continue
        source_chunk_file = paper_dir_level2 / f"{pid}_content_list.json"
        target_chunk_file = chunks_output_dir / f"{pid}_chunks.json"
...
if __name__ == "__main__":
    ...
    prepare_chunks(mineru_dir, chunks_dir, paper_id) 